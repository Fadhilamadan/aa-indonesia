...
$sqlClosing = "SELECT * FROM closing";
$resultClosing = mysqli_query($link, $sqlClosing);
//$rowClosing = mysqli_fetch_array($resultClosing);
if(!$resultClosing) {
    echo "SQL ERROR: ".$sqlClosing; 
}
            ...
            <nav class="navbar navbar-default">
                <div class="container-fluid">
                    <div class="collapse navbar-collapse">
                        ...
                        <a class="navbar-brand" href="rekapitulasi.php">Laporan Rekapitulasi</a>
                                ...
                                <div class="card-content">
                                    <div class="row">
                                        ...
                                        <div class="col-md-2">
                                            ...
                                            <div class="form-group"> 
                                                <input type="submit" name="submit" id="cari" class="btn btn-info btn-fill pull-right" value="Cari" style="width: 150px;"/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="m-b-20 table-responsive">
                    <table id="datatable-buttons" class="table table-striped">
                        <thead>
                            ...
                            <th data-field="id_closer" data-sortable="true">ID Closer</th>
                            ID orang yang closing
                            <th data-field="komisi" data-sortable="true">Komisi</th>
                            <th data-field="principal" data-sortable="true">Principal</th>
                            <th data-field="vice_principal" data-sortable="true">V. Principal</th>
                            <th data-field="upline1" data-sortable="true">Upline 1</th>
                            <th data-field="upline2" data-sortable="true">Upline 2</th>
                            <th data-field="upline3" data-sortable="true">Upline 3</th>
                        ...
                        <tbody>
                            ...
                            $hitung = 1;
                            $index = 0;
                            while ($rowClosing = mysqli_fetch_object($resultClosing)) {

                                $sqlHubKomisi = "SELECT * FROM hub_komisi WHERE closing_id=".$rowClosing->id;
                                $resultHubKomisi = mysqli_query($link, $sqlHubKomisi);
                                //$rowHubKomisi = mysqli_fetch_array($resultHubKomisi);
                                if(!$resultHubKomisi) {
                                    echo "SQL ERROR: ".$sqlHubKomisi; 
                                }
                                ...
                                //$rowHubClosing = mysqli_fetch_array($resultHubClosing);
                                if(!$resultHubClosing) {
                                    echo "SQL ERROR: ".$sqlHubClosing; 
                                }

                                while($rowHubClosing=mysqli_fetch_object($resultHubClosing)){
                                    $principal=0;
                                    $vice_principal=0;
                                    $upline1=0;
                                    $upline2=0;
                                    $upline3=0;
                                    $total=0;
                                    $sqlCekKP = "SELECT * FROM hub_komisi WHERE jenis='pasif - principal' AND closing_id=".$rowClosing->id;
                                    $resultCekKP = mysqli_query($link, $sqlCekKP);
                                    $rowCekKP = mysqli_fetch_array($resultCekKP);
                                    $sqlCekKVP = "SELECT * FROM hub_komisi WHERE jenis='pasif - vice principal' AND closing_id=".$rowClosing->id;
                                    $resultCekKVP = mysqli_query($link, $sqlCekKVP);
                                    $rowCekKVP = mysqli_fetch_array($resultCekKVP);
                                    $sqlCekKUP1 = "SELECT * FROM hub_komisi WHERE jenis='pasif - upline1' AND closing_id=".$rowClosing->id;
                                    $resultCekKUP1 = mysqli_query($link, $sqlCekKUP1);
                                    $rowCekKUP1 = mysqli_fetch_array($resultCekKUP1);
                                    $sqlCekKUP2 = "SELECT * FROM hub_komisi WHERE jenis='pasif - upline2' AND closing_id=".$rowClosing->id;
                                    $resultCekKUP2 = mysqli_query($link, $sqlCekKUP2);
                                    $rowCekKUP2 = mysqli_fetch_array($resultCekKUP2);
                                    $sqlCekKUP3 = "SELECT * FROM hub_komisi WHERE jenis='pasif - upline3' AND closing_id=".$rowClosing->id;
                                    $resultCekKUP3 = mysqli_query($link, $sqlCekKUP3);
                                    $rowCekKUP3 = mysqli_fetch_array($resultCekKUP3);

                                    if(mysqli_num_rows($resultCekKP)>0){
                                        $principal=$rowCekKP['nominal'];
                                    }
                                    if(mysqli_num_rows($resultCekKVP)>0){
                                        $vice_principal=$rowCekKVP['nominal'];
                                    }
                                    if(mysqli_num_rows($resultCekKUP1)>0){
                                        $upline1=$rowCekKUP1['nominal'];
                                    }
                                    if(mysqli_num_rows($resultCekKUP2)>0){
                                        $upline2=$rowCekKUP2['nominal'];
                                    }
                                    if(mysqli_num_rows($resultCekKUP3)>0){
                                        $upline3=$rowCekKUP3['nominal'];
                                    }
                                    $total = $principal+$vice_principal+$upline1+$upline2+$upline3;
                                    echo "<tr data-index='".$rowClosing->id."'>";
                                    echo "<td>" . $hitung. "</td>";
                                    echo "<td>" . $rowClosing->tanggal . "</td>";
                                    echo "<td>" . $rowHubClosing->employee_id . "</td>";
                                    echo "<td>" . $rowHubClosing->komisi. "</td>";
                                    echo "<td>" . $principal . "</td>";
                                    echo "<td>" . $vice_principal . "</td>";
                                    echo "<td>" . $upline1 . "</td>";
                                    echo "<td>" . $upline2 . "</td>";
                                    echo "<td>" . $upline3 . "</td>";
                                    echo "<td>" . $total . "</td>";
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="fresh-datatables">
                                                <table id="datatable-buttons" class="table table-striped table-no-bordered table-hover dataTable dtr-inline">
                                                    <thead>
                                                        <th>Id_closer</th>
                                                        <th>Tanggal</th>
                                                        <th>Komisi</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#cari').click(function(){
                ...
                $.ajax({
                    type: 'post',
                    url: '../proses.php?cmd=laporan_rekapitulasi',
                    data: {
                    ...
                    success: function (datax) {
                        ...
                        for(var i =0; i<json.length;i++){
                            table.rows.add([{
                                'id_closer': json[i].id_closer,
                                'tanggal': json[i].tanggal,
                                'komisi': fGanti(+json[i].komisi),
                                'principal': fGanti(+json[i].principal),
                                'vice_principal': fGanti(+json[i].vice_principal),
                                'upline1': fGanti(+json[i].upline1),
                                'upline2': fGanti(+json[i].upline2),
                                'upline3': fGanti(+json[i].upline3),
                                'total': fGanti(+json[i].total),
                            }]);
                        }
                        table.draw();
